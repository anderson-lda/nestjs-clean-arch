...
describe("InMemoryRepository unit tests", () => {
  ...
  describe('applyFilter method',()=>{
    it("should no filter items filter param is null", async ()=>{
      const items = [new StubEntity({name: 'name value', price: 50})]
      const spyFilterMethod = jest.spyOn(items,'filter') //espia se algo foi chamado
      const itemsFiltered = await sut['applyFilter'](items,null) //usando array pega-se todas as propriedades, não só as públicas
    ...
    it("should filter using filter param", async ()=>{
      ...
      const spyFilterMethod = jest.spyOn(items,'filter') //espia se algo foi chamado
      let itemsFiltered = await sut['applyFilter'](items,'TEST') //usando array pega-se todas as propriedades, não só as públicas
      expect(itemsFiltered).toStrictEqual([items[0], items[1]])
      expect(spyFilterMethod).toHaveBeenCalledTimes(1)

      itemsFiltered = await sut['applyFilter'](items,'test') //usando array pega-se todas as propriedades, não só as públicas
      expect(itemsFiltered).toStrictEqual([items[0], items[1]])
      expect(spyFilterMethod).toHaveBeenCalledTimes(2)

      itemsFiltered = await sut['applyFilter'](items,'no-filter') //usando array pega-se todas as propriedades, não só as públicas
  ...
  describe('applySort method',()=>{
    it("should not sort items", async ()=>{
      const items = [new StubEntity({name: 'b', price: 50}),
  ...
  describe('search method',()=>{
    it("should only apply pagination when the others params are null", async ()=>{
      const entity = new StubEntity({name: 'test', price: 50})
      const items = Array(16).fill(entity) //16 cópias de entity serão criadas
      sut.items = items
    ...
    it("should search using apply paginate, sort and filter", async ()=>{
      const items = [
        new StubEntity({name: 'test', price: 50}),
        new StubEntity({name: 'a', price: 50}),
        new StubEntity({name: 'TEST', price: 50}),
